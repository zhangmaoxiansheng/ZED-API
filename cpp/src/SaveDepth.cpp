...
void saveL_R(Camera& zed,int frame_num)
{
    Resolution image_size = zed.getResolution();
    Mat buffer_sl;
    cv::Mat buffer_cv;
    ...
    cv::imwrite("./images_manual/" + std::to_string(frame_num) + "_l.png",left_image);
    cv::imwrite("./images_manual/" + std::to_string(frame_num) + "_r.png",right_image);
